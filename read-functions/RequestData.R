...
RequestData <- function(
  ...
  datasets_info <- lapply(
    dataset_info_list,
    function(ds_info) {
      info(ds_info$datasetid, ds_info$url)
    }
  )
  ...
  queries <- lapply(
    ...
    function(d) {
      query <- FormatDayQuery(
        d,
        ds_info_list = datasets_info,
        latitude_range,
        longitude_range
      )
      rerddap_var_names <- query$datasetx$variables$variable_name
      qq <- list(
        query_list = query,
        var_name = var_name[
          which(var_name %in% rerddap_var_names)
        ],
